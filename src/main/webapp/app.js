...
function applyAnalysisToComments(comments, analysis) {
  ...
  return comments.map((comment, index) => {
    ...
    return {
      ...
      quadrant: detail.quadrant || point.quadrant || point.label || comment.quadrant || quadrantFor(comment.text || "", Number(comment.ratingValue || 3), Number(point.x || 0))
    };
  });
}
...
function renderCharts() {
  ...
  // 象限背景色块
  const quadRects = [
    `<rect x="34" y="24" width="146" height="82" fill="rgba(220,105,102,0.10)"/>`,   // leftTop
    `<rect x="180" y="24" width="146" height="82" fill="rgba(220,170,55,0.10)"/>`,    // rightTop
    `<rect x="34" y="106" width="146" height="82" fill="rgba(144,143,139,0.08)"/>`,  // leftBottom
    `<rect x="180" y="106" width="146" height="82" fill="rgba(82,196,26,0.08)"/>`,   // rightBottom
  ].join("");
  ...
  const points = currentAnalysis.scatter.map((point, idx) => {
    ...
    const r = Math.max(3, Math.min(9, (point.weight || 5) * 0.6));
    const quote = point.quote ? escapeHtml(point.quote) : "";
    const elabel = point.emotionLabel ? escapeHtml(point.emotionLabel) : "";
    return `<circle cx="${cx.toFixed(1)}" cy="${cy.toFixed(1)}" r="${r.toFixed(1)}" fill="${fill}" fill-opacity="0.75" style="animation-delay:${idx * 25}ms"><title>${elabel}｜${point.sentiment}｜${quote}</title></circle>`;
  }).join("");

  // 情绪重心
  const cx = 180 + (centroid.x || 0) * 130;
  const cy = 188 - (centroid.y || 0) * 156;
  const centroidMarker = `
    <line x1="${cx - 10}" y1="${cy.toFixed(1)}" x2="${cx + 10}" y2="${cy.toFixed(1)}" stroke="#fffaf4" stroke-width="2"/>
    <line x1="${cx.toFixed(1)}" y1="${cy - 10}" x2="${cx.toFixed(1)}" y2="${cy + 10}" stroke="#fffaf4" stroke-width="2"/>
    <circle cx="${cx.toFixed(1)}" cy="${cy.toFixed(1)}" r="5" fill="none" stroke="#fffaf4" stroke-width="2"/>
  `;
  ...
  $("#scatter-chart").innerHTML = `
    <div class="emotion-map">
      <svg viewBox="0 0 360 220" role="img" aria-label="观众情绪分布图">
        ...
        ${quadLabels}
        <text x="180" y="18" text-anchor="middle" fill="#5e5d59" font-size="9">← 消极    情绪倾向    积极 →</text>
        <text x="10" y="106" fill="#5e5d59" font-size="9" transform="rotate(-90 10 106)">强烈 ↑  情绪强度  ↓ 平静</text>
        ${points}
        ${centroidMarker}
        <text x="${(cx + 8).toFixed(1)}" y="${(cy + 4).toFixed(1)}" fill="#fffaf4" font-size="9" font-weight="700">重心</text>
      </svg>
      <div class="emotion-legend">
        <span><span class="dot" style="background:#dcaa37"></span>正面</span>
        <span><span class="dot" style="background:#dc6966"></span>负面</span>
        <span><span class="dot" style="background:#908f8b"></span>中性</span>
        <span>点大小=情绪强度</span>
        <span>✛=情绪重心</span>
      </div>
      ${emSummary.distributionSummary ? `<div class="emotion-summary"><strong>主导情绪：</strong>${escapeHtml(emSummary.dominantEmotion || "")} — ${escapeHtml(emSummary.distributionSummary)}。${escapeHtml(emSummary.interpretation || "")}</div>` : ""}
    </div>
  `;
