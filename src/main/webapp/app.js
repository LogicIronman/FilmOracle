...
function getSettings() {
  try {
    const saved = JSON.parse(localStorage.getItem("filmoracle-settings") || "{}");
    return {
      apiBase: saved.apiBase || "/api",
      commentCount: saved.commentCount || 100,
      timeout: saved.timeout || "9s",
      model: saved.model || "moonshot-v1-8k",
      apiKey: saved.apiKey || "",
      prompt: saved.prompt || DEFAULT_PROMPT,
      fallback: saved.fallback !== false
    };
  } catch {
    return { apiBase: "/api", commentCount: 100, timeout: "9s", model: "moonshot-v1-8k", apiKey: "", prompt: DEFAULT_PROMPT, fallback: true };
  }
}
...
function saveSettings() {
  const settings = {
    ...
    prompt: $("#setting-prompt").value,
    fallback: $("#setting-fallback").checked
  };
  localStorage.setItem("filmoracle-settings", JSON.stringify(settings));
...
function loadSettingsToForm() {
  ...
  $("#setting-prompt").value = s.prompt;
  $("#setting-fallback").checked = s.fallback;
}
...
function loadHistory() {
  try {
    return JSON.parse(localStorage.getItem("filmoracle-history") || "[]");
  } catch {
    return [];
  }
}
...
function persistHistory() {
  localStorage.setItem("filmoracle-history", JSON.stringify(history.slice(0, 20)));
}
...
async function loadCommentsAndAnalyze(runAnalysis = false) {
  ...
  if (runAnalysis) {
    ...
    if (hasApiKey) {
      appendTask(`> [2/3] 使用 AI 分析 (${settings.model})`);
      appendTask(`> > 发送 ${currentComments.length} 条评论到 AI...`);
      appendTask(`> > 提示词长度: ${settings.prompt.length} 字符`);
    } else {
