...
对筛选出的评论做方面级情感分析，输出一个数组，每个元素代表一条评论针对某一个评价方面的情感判断。结构如下：
...
- votes：该评论的点赞数（整数，没有则填0）
- text：评论原文摘录，保留原话，最多60字
...

{
  ...
  "scatter": [{"aspect": "剧情|演技|视听|节奏|主题|配乐|美术|结尾", "polarity": -5.0到5.0, "intensity": 0.0到5.0, "votes": 点赞数, "text": "评论原文摘录最多60字"}],
  "emotionMap": {
    "quadrants": [
...
function renderCharts() {
  ...
  const points = currentAnalysis.scatter.map((point, idx) => {
    ...
    const r = Math.max(3, Math.min(10, 3 + (point.votes || 0) / 20));
    const text = point.text ? escapeHtml(point.text.substring(0, 40)) : "";
    return `<circle cx="${cx.toFixed(1)}" cy="${cy.toFixed(1)}" r="${r.toFixed(1)}" fill="${fill}" fill-opacity="0.75" style="animation-delay:${idx * 20}ms"><title>${escapeHtml(aspect)}｜polarity=${point.polarity}｜intensity=${point.intensity}｜${text}</title></circle>`;
  }).join("");
