...
对评论做方面级情感分析，将情感相似的评论聚合为散点。每条评论涉及多个方面就拆分到对应方面，然后将相同方面且情感相近（polarity差≤1.0，intensity差≤1.0）的评论聚合为一个点。
- aspect：评价方面，枚举值为 剧情|演技|视听|节奏|主题|配乐|美术|结尾
- polarity：-5.0 到 5.0 的浮点数，负值=差评，正值=好评，0=中性
- intensity：0.0 到 5.0 的浮点数，0=平淡客观，5=强烈情绪（封神/崩坏/震撼/败笔）
- votes：该点聚合的相似评论数量（有多少条评论对该方面表达了相近的情感，不是点赞数）。votes越大代表越多观众持相同看法。
...
- 至少输出20条scatter，评论数量多时应输出更多（50条以上为佳）。每个方面都应有至少1个点。确保散点覆盖4个象限。
...
// ── 前端本地散点（方面级聚合）──
const ASPECT_REGEX = {
  "剧情": /故事|情节|叙事|剧本|线索|伏笔|反转|逻辑|设定|桥段|套路|开头|铺垫|世界观|虐心|催泪|狗血|俗套/,
  "演技": /演技|表演|演员|角色|饰演|演绎|主角|配角|群像|刻画|塑造|眼神|微表情|出戏|入戏|台词功|演得/,
  "视听": /画面|镜头|摄影|视效|特效|色彩|构图|光影|视觉|大片感|视觉冲击|美学|质感|名场面|视觉盛宴/,
  "节奏": /节奏|拖沓|紧凑|缓慢|快|慢|剪辑|转折|推进|松散|冗长|注水|拖戏|拖拉|太长|高能|一气呵成/,
  "主题": /主题|思想|深度|内涵|隐喻|象征|哲学|意义|表达|探讨|女性|男权|社会|现实|阶层|批判|反思|启示|价值观/,
  "配乐": /配乐|音乐|音效|BGM|主题曲|背景音|原声|OST|音乐性/,
  "美术": /美术|布景|服装|道具|场景|美术设计|造型|服化道|审美|妆造|年代感/,
  "结尾": /结尾|结局|最后|收尾|尾声|终章|收场|落幕|HE|BE|开放式/
};
...
function buildLocalScatter(comments) {
  ...
  for (const comment of comments) {
    ...
    for (const [aspect, regex] of Object.entries(ASPECT_REGEX)) {
      ...
      polarity += (pos - neg) * 0.4;
      polarity = Math.max(-5, Math.min(5, Math.round(polarity * 2) / 2));

      const intensityWords = pos + neg + countTerms(text, ["震撼", "封神", "绝", "崩", "烂", "败", "哭", "泪", "破防", "窒息", "压抑", "愤怒", "爽", "燃"]);
      let intensity = 1.0 + intensityWords * 0.6;
      if (strongPos || strongNeg) intensity += 1.0;
      intensity = Math.max(0.5, Math.min(5, Math.round(intensity * 2) / 2));
